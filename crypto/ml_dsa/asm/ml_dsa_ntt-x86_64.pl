...
#
#   The routines here are vectorized using AVX2 instructions, performing
...
#   ensuring that this file provides a drop-in, high-performance backend using the
...
#
# Notes:
#   - Relies on AVX2 SIMD instructions
#   - Must be kept functionally synchronized with the math and interface of ml_dsa_ntt.c
#   - Data structures, twiddle factors ("zetas"), and constants must match those in the C implementation
###############################################################################
...
#
# Description:
#   Multiplies two 8-element vectors of 32-bit unsigned integers modulo Q using
#   Montgomery reduction. The inputs (A and B) are in YMM registers, where each
#   register contains 8 packed 32-bit integers. The result, (A * B mod Q), is also
#   stored in a YMM register. This routine is used as the core modular
#   multiplication step in NTT butterfly operations.
#
# Parameters:
#   inA      - Input YMM containing 8 packed 32-bit unsigned values (A)
#   inB      - Input YMM containing 8 packed 32-bit unsigned values (B)
...
#
# Description:
#   Performs the butterfly operation for a single NTT stage on two input vectors,
...
#
# Description:
#   Performs the first three layers (levels 0, 1, and 2) of the NTT (Number Theoretic Transform)
#   in-place.
#   Each group consists of 8 x 32-bit coefficients, and 8 such groups are processed with offsets
#   separated by 32 x 4 bytes (128 bytes). Each group undergoes consecutive butterfly operations
#   with the appropriate "zetas" (twiddle factors) for each level. This function must be called
#   4 times with different offsets to process all 256 coefficients.
#
# Layer/level details:
...
#
# Notes:
#   - Should be invoked 4 times for complete polynomial: offsets 0, 8*4, 16*4, 24*4
#   - Utilizes ntt_butterfly for stage-wise modular butterfly combining and reduction.
###############################################################################
...
#
# Description:
#   Performs layers 3 through 7 of the NTT (Number Theoretic Transform) in-place.
#   Each set of 32 32-bit words is laid out contiguously, and the function is called 8 times
#   for a full NTT of 256 coefficients, with appropriate offsets. At each level, the function
#   executes butterfly operations in the correct pattern and applies the corresponding twiddle
#   factors ("zetas").
#
# Layer/level details:
...
#
# Output:
#   In-place NTT-transformed coefficients for the affected group.
#
# Notes:
#   - Should be called 8 times per complete 256-coefficient transform (offset 0, 32*4, ... 224*4).
...
___
    &ntt_butterfly("%ymm0", "%ymm1", "%ymm12", "%ymm0", "%ymm1",    # w_even, w_odd, zetas, n_even, n_odd
                   "%ymm4", "%ymm5", "%ymm6", "%ymm7",              # tmp
                   "%ymm14", "%ymm15");                             # qinv, q
    &ntt_butterfly("%ymm2", "%ymm3", "%ymm13", "%ymm2", "%ymm3",    # w_even, w_odd, zetas, n_even, n_odd
                   "%ymm4", "%ymm5", "%ymm6", "%ymm7",              # tmp
    ...
    # load zetas
    vpbroadcastd $l5(%r11), %ymm12
    vpbroadcastd $l5+4(%r11), %ymm11
    vpblendd \$0xf0, %ymm11, %ymm12, %ymm12
    vpbroadcastd $l5+8(%r11), %ymm13
    vpbroadcastd $l5+12(%r11), %ymm11
    vpblendd \$0xf0, %ymm11, %ymm13, %ymm13
    ...
    #   ymm4 = [ 0  1  2  3 |  8  9 10 11] (even)
    #   ymm6 = [ 4  5  6  7 | 12 13 14 15] (odd)
    vperm2i128 \$0x20, %ymm1, %ymm0, %ymm4
    vperm2i128 \$0x31, %ymm1, %ymm0, %ymm6
    vperm2i128 \$0x20, %ymm3, %ymm2, %ymm5
    vperm2i128 \$0x31, %ymm3, %ymm2, %ymm7
___
    &ntt_butterfly("%ymm4", "%ymm6", "%ymm12", "%ymm0", "%ymm2",    # w_even, w_odd, zetas, n_even, n_odd
                   "%ymm8", "%ymm9", "%ymm10", "%ymm11",            # tmp
                   "%ymm14", "%ymm15");                             # qinv, q
    &ntt_butterfly("%ymm5", "%ymm7", "%ymm13", "%ymm1", "%ymm3",    # w_even, w_odd, zetas, n_even, n_odd
                   "%ymm8", "%ymm9", "%ymm10", "%ymm11",            # tmp
                   "%ymm14", "%ymm15");                             # qinv, q
    $code .= <<___;
    ...
    #   ymm2 = [ 4  5  6  7] [12 13 14 15] (odd)
    #   ymm1 = [16 17 18 19] [24 25 26 27] (even)
    #   ymm3 = [20 21 22 23] [28 29 30 31] (odd)
    # Desired DWORD layout:
    #   ymm4 = [ 0  1  4  5] [ 8  9 12 13] (even)
    #   ymm6 = [ 2  3  6  7] [10 11 14 15] (odd)
    vpunpcklqdq %ymm2, %ymm0, %ymm4 # w_even[0:7]
    vpunpckhqdq %ymm2, %ymm0, %ymm6 # w_odd[0:7]
    vpunpcklqdq %ymm3, %ymm1, %ymm5 # w_even[8:15]
    vpunpckhqdq %ymm3, %ymm1, %ymm7 # w_odd[8:15]

    # load zetas
    vmovdqu $l6(%r11), %ymm10       # 0 1 2 3 | 4 5 6 7
    vpshufd \$0xfa, %ymm10, %ymm11  # 2 2 3 3 | 6 6 7 7
    vpshufd \$0x50, %ymm10, %ymm10  # 0 0 1 1 | 4 4 5 5
    vperm2i128 \$0x20, %ymm11, %ymm10, %ymm12   # 0 0 1 1 | 2 2 3 3
    vperm2i128 \$0x31, %ymm11, %ymm10, %ymm13   # 4 4 5 5 | 6 6 7 7
...
___
    ...
    #   ymm2 = [ 2  3  6  7] [10 11 14 15]
    # Required DWORD layout:
    #   ymm0 = [ 0  2  4  6] [ 8 10 12 14] (even)
    #   ymm2 = [ 1  3  5  7] [ 9 11 13 15] (odd)

    vpshufd \$0x88, %ymm0, %ymm4      # [0 4 0 4] | [ 8 12  8 12]
    vpshufd \$0x88, %ymm2, %ymm5      # [2 6 2 6] | [10 14 10 14]
    vpshufd \$0xDD, %ymm0, %ymm6      # [1 5 1 5] | [ 9 13  9 13]
    vpshufd \$0xDD, %ymm2, %ymm7      # [3 7 3 7] | [11 15 11 15]
    vpunpckldq %ymm5, %ymm4, %ymm0    # [0 2 4 6] | [ 8 10 12 14]
    vpunpckldq %ymm7, %ymm6, %ymm2    # [1 3 5 7] | [ 9 11 13 15]

    vpshufd \$0x88, %ymm1, %ymm4
    vpshufd \$0x88, %ymm3, %ymm5
    vpshufd \$0xDD, %ymm1, %ymm6
    vpshufd \$0xDD, %ymm3, %ymm7
    vpunpckldq %ymm5, %ymm4, %ymm1
    vpunpckldq %ymm7, %ymm6, %ymm3

    # load zetas
    vmovdqu $l7(%r11), %ymm12
    vmovdqu $l7+32(%r11), %ymm13
___
    &ntt_butterfly("%ymm0", "%ymm2", "%ymm12", "%ymm0", "%ymm2",    # w_even, w_odd, zetas, n_even, n_odd
                   "%ymm4", "%ymm5", "%ymm6", "%ymm7",              # tmp
                   "%ymm14", "%ymm15");                             # qinv, q
    &ntt_butterfly("%ymm1", "%ymm3", "%ymm13", "%ymm1", "%ymm3",    # w_even, w_odd, zetas, n_even, n_odd
                   "%ymm4", "%ymm5", "%ymm6", "%ymm7",              # tmp
    ...
    # new w_odd = ymm2, ymm3, new w_even = ymm0, ymm1
    # Inputs:
    #   w_even: %ymm0 (e0..e7), %ymm1 (e8..e15)
    #   w_odd : %ymm2 (o0..o7), %ymm3 (o8..o15)
    # Output:
    #   (e0,o0,e1,o1,..e15,o15)

    vpunpckldq %ymm2, %ymm0, %ymm4      # ymm4 = [e0,o0,e1,o1 | e4,o4,e5,o5]
    vpunpckhdq %ymm2, %ymm0, %ymm5      # ymm5 = [e2,o2,e3,o3 | e6,o6,e7,o7]

    # ymm6: [ymm4.low | ymm5.low] = [e0,o0,e1,o1 | e2,o2,e3,o3]
    # ymm7: [ymm4.high | ymm5.high] = [e4,o4,e5,o5 | e6,o6,e7,o7]
    vperm2i128 \$0x20, %ymm5, %ymm4, %ymm6
    vperm2i128 \$0x31, %ymm5, %ymm4, %ymm7
    vmovdqu %ymm6, $off(%rdi)
    vmovdqu %ymm7, $off+32(%rdi)

    vpunpckldq  %ymm3, %ymm1, %ymm4     # ymm4 = [e8,o8,e9,o9 | e12,o12,e13,o13]
    vpunpckhdq  %ymm3, %ymm1, %ymm5     # ymm5 = [e10,o10,e11,o11 | e14,o14,e15,o15]

    # ymm6: [ymm4.low | ymm5.low] = [e8,o8,e9,o9 | e10,o10,e11,o11]
    # ymm7: [ymm4.high | ymm5.high] = [e12,o12,e13,o13 | e14,o14,e15,o15]
    vperm2i128 \$0x20, %ymm5, %ymm4, %ymm6
    vperm2i128 \$0x31, %ymm5, %ymm4, %ymm7
    vmovdqu %ymm6, $off+64(%rdi)
    vmovdqu %ymm7, $off+96(%rdi)
___
}
...
#
# Description:
#   Performs the butterfly operation for a single stage of the inverse
#   Number Theoretic Transform (INTT) on two 8-element vectors of 32-bit integers,
#   using AVX2 vector instructions. Implements the core data-mixing and modular
#   multiplication steps with a zeta (twiddle factor) for the INTT, including the
#   necessary modular reductions. The updated even and odd results are written
#   back to the designated registers.
#
#   Specifically, this butterfly operation computes:
...
#
# Parameters:
#   w_even    - Register with 8 packed 32-bit "even" input coefficients
#   w_odd     - Register with 8 packed 32-bit "odd" input coefficients
#   zetas     - Register with 8 packed 32-bit modular twiddle factors for this butterfly
#   tmp0-tmp2 - Scratch registers for temporary/intermediate values
#   n_even    - Output register for new even coefficients (updated in-place)
#   n_odd     - Output register for new odd coefficients (updated in-place)
#   q_neg_inv - Register with -Q^{-1} mod 2^32 for Montgomery reduction
#   q         - Register with modulus Q
#
# Output:
#   n_even, n_odd - (overwritten in caller registers) with reduced updated values
#
# Side effects:
#   Clobbers tmp0, tmp1, tmp2
###############################################################################
...
#
# Description:
#   Executes the first five stages (levels 0–4) of the inverse Number Theoretic Transform (INTT)
#   on groups of 32 coefficients in-place.
#   This function hierarchically mixes and transforms groups of coefficients using modular
#   butterfly operations and stage-specific zeta (twiddle) factors, performing all required
#   re-packing and permutations for each layer.
#
#   Each call operates on a block of 32 coefficients, and must be repeated 8 times (with
#   offsets 0, 32*4, 64*4, ..., 224*4) to process all 256 coefficients in a polynomial.
#
# Layer/Level details:
...
#
# Notes:
#   - Function must be called 8 times for a full 256-coefficient INTT layer sweep.
###############################################################################
...
sub intt_levels0to4 {
    ...
    # load w_even and w_odd together
    vmovdqu $off(%rdi), %ymm4
    vmovdqu $off+32(%rdi), %ymm5

    # compact even words into ymm0 (w_even[0:7])
    vmovdqa idx_even(%rip), %ymm11
    vpermd %ymm4, %ymm11, %ymm2             # [ 0, 2, 4, 6 | 0, 2, 4, 6]
    vpermd %ymm5, %ymm11, %ymm3             # [ 8,10,12,14 | 8,10,12,14]
    vpblendd \$0xf0, %ymm3, %ymm2, %ymm0    # [ 0, 2, 4, 6 | 8,10,12,14]

    # compact odd words into ymm1 (w_odd[0..7])
    vmovdqa idx_odd(%rip), %ymm11
    vpermd %ymm4, %ymm11, %ymm2             # [ 1, 3, 5, 7 | 1, 3, 5, 7]
    vpermd %ymm5, %ymm11, %ymm3             # [ 9,11,13,15 | 9,11,13,15]
    vpblendd \$0xf0, %ymm3, %ymm2, %ymm1    # [ 1, 3, 5, 7 | 9,11,13,15]
...
___
    ...
    &intt_butterfly("%ymm0", "%ymm1", "%ymm4",   # w_even, w_odd, zetas
                    "%ymm11", "%ymm12", "%ymm13", # temporary
...
$code .= <<___;
    ...
    # load w_even and w_odd together
    vmovdqu $off+64(%rdi), %ymm4
    vmovdqu $off+96(%rdi), %ymm5

    # compact even words into ymm2 (w_even[8..15])
    vmovdqa idx_even(%rip), %ymm11
    vpermd %ymm4, %ymm11, %ymm6
    vpermd %ymm5, %ymm11, %ymm7
    vpblendd \$0xf0, %ymm7, %ymm6, %ymm2
    ...
    # load 16 zetas
    vmovdqu $l0+32(%r11), %ymm4

___

    &intt_butterfly("%ymm2", "%ymm3", "%ymm4",   # w_even, w_odd, zetas
                    "%ymm11", "%ymm12", "%ymm13", # temporary
...
$code .= <<___;
    ...
    vpunpckldq %ymm1, %ymm0, %ymm4     # A = [0,1,2,3 | 8,9,10,11]
    vpunpckhdq %ymm1, %ymm0, %ymm5     # B = [4,5,6,7 | 12,13,14,15]

    # Per 128-bit lane, select pairs:
    # imm=0x44 -> [A0,A1,B0,B1] -> [0,1,4,5 | 8,9,12,13]
    vshufps \$0x44, %ymm5, %ymm4, %ymm0

    # imm=0xee -> [A2,A3,B2,B3] -> [2,3,6,7 | 10,11,14,15]
    vshufps \$0xee, %ymm5, %ymm4, %ymm1

    # load 4 zetas and populate across ymm
    vmovdqu $l1(%r11), %xmm4                # [0 1 2 3]
    vpmovzxdq %xmm4, %ymm4
    vmovsldup %ymm4, %ymm4                  # [0 0 1 1] [2 2 3 3]
___
...
$code .= <<___;
    ...
    # Interleave even/odd within each 128-bit lane:
    vpunpckldq %ymm3, %ymm2, %ymm4     # A = [0,1,2,3 | 8,9,10,11]
    vpunpckhdq %ymm3, %ymm2, %ymm5     # B = [4,5,6,7 | 12,13,14,15]
    ...
    # imm=0xee -> [A2,A3,B2,B3] -> [2,3,6,7 | 10,11,14,15]
    vshufps \$0xee, %ymm5, %ymm4, %ymm3

    # load 4 zetas and populate across YMM
    vmovdqu $l1+16(%r11), %xmm4             # [0 1 2 3]
    vpmovzxdq %xmm4, %ymm4
    vmovsldup %ymm4, %ymm4                  # [0 0 1 1] [2 2 3 3]
___
...
$code .= <<___;
    ...
    # ymm9 = [4,5,6,7 | 12,13,14,15]
    vshufps \$0xee, %ymm1, %ymm0, %ymm9

    # broadcast zetas
    vpbroadcastd $l2(%r11), %ymm4
    vpbroadcastd $l2+4(%r11), %ymm5
    vpblendd \$0xf0, %ymm5, %ymm4, %ymm4
___

    &intt_butterfly("%ymm8", "%ymm9", "%ymm4",   # w_even, w_odd, zetas
                    "%ymm11", "%ymm12", "%ymm13", # temporary
                    "%ymm0", "%ymm1",            # n_even, n_odd
                    "%ymm14", "%ymm15");         # qinv, q32
...
$code .= <<___;
    ...
    vshufps \$0xee, %ymm3, %ymm2, %ymm9

    # broadcast zetas
    vpbroadcastd $l2+8(%r11), %ymm4
    vpbroadcastd $l2+12(%r11), %ymm5
    vpblendd \$0xf0, %ymm5, %ymm4, %ymm4
___
...
$code .= <<___;
    ...
    vperm2i128 \$0x31, %ymm1, %ymm0, %ymm9      # [8,9,10,11 | 12,13,14,15]

    # broadcast zetas
    vpbroadcastd $l3(%r11), %ymm4
___
...
$code .= <<___;
    ...
    # broadcast zetas
    vpbroadcastd $l4(%r11), %ymm4
___

    &intt_butterfly("%ymm0", "%ymm2", "%ymm4",   # w_even, w_odd, zetas
                    "%ymm11", "%ymm12", "%ymm13", # temporary
                    "%ymm0", "%ymm2",            # n_even, n_odd
                    "%ymm14", "%ymm15");         # qinv, q32

    &intt_butterfly("%ymm1", "%ymm3", "%ymm4",   # w_even, w_odd, zetas
                    "%ymm11", "%ymm12", "%ymm13", # temporary
...
#
# Description:
#   Processes the last three stages (levels 5–7) of the inverse Number Theoretic Transform (INTT)
#   in-place.
#   This function completes the hierarchical merging of INTT, applying stage-specific zeta
...
#
#   Each call processes groups of 8 coefficients at 32 coefficients offset.
#   It should be invoked 4 times with offsets equal to 0*4, 8*4, 16*4 and 24*4 to cover
...
#     all 256 coefficients.
#   - Each butterfly step uses appropriate zeta factors from table for the current level.
#   - The final Montgomery product scaling is applied to all outputs to complete the inverse.
###############################################################################
...
#       const uint32_t *src_a,   // (rdi) Input polynomial A (in NTT domain)
#       const uint32_t *src_b,   // (rsi) Input polynomial B (in NTT domain)
#       uint32_t *dst,           // (rdx) Output polynomial (result)
#   );
#
# Description:
...
#
# Inputs:
#   src_a - First input polynomial, NTT domain
#   src_b - Second input polynomial, NTT domain
#   dst   - Destination buffer for output coefficients
#
# Output:
#   - Output buffer 'dst' contains the coefficient-wise modular product of
...
.align 32
.Lmult_loop:
    # Load src_a and src_b into ymm registers
...
#       uint32_t *coeffs,      // Pointer to coefficients (input: normal domain, output: NTT domain)
#       const uint32_t *zetas  // Pointer to zeta (twiddle factor) table for the forward NTT
#   );
#
# Description:
...
#
# Inputs:
#   coeffs - Pointer to the coefficient array (will be overwritten in-place by the NTT result)
#   zetas  - Pointer to the precomputed table of forward NTT zeta (twiddle) factors (from ml_dsa_ntt.c)
#
# Output:
#   - The 'coeffs' array is updated in-place with the corresponding NTT-domain representation.
###############################################################################
...
___
    ...
    # offset arguments: coeff, l3, l4, l5, l6, l7
    &ntt_levels3to7(  0*4,  8*4,  16*4, 32*4,  64*4, 128*4);
    &ntt_levels3to7( 32*4,  9*4,  18*4, 36*4,  72*4, 144*4);
    &ntt_levels3to7( 64*4, 10*4,  20*4, 40*4,  80*4, 160*4);
    &ntt_levels3to7( 96*4, 11*4,  22*4, 44*4,  88*4, 176*4);
    &ntt_levels3to7(128*4, 12*4,  24*4, 48*4,  96*4, 192*4);
    &ntt_levels3to7(160*4, 13*4,  26*4, 52*4, 104*4, 208*4);
    &ntt_levels3to7(192*4, 14*4,  28*4, 56*4, 112*4, 224*4);
    &ntt_levels3to7(224*4, 15*4,  30*4, 60*4, 120*4, 240*4);
...
#        uint32_t *coeffs // (rdi) Pointer to coefficients
...
#
# Inputs:
#   coeffs - Pointer to the polynomial coefficient array (in-place transform)
#
# Output:
#   - The 'coeffs' array is updated in-place to contain the standard domain polynomial.
#   - Uses 'zetas_inverse' table
###############################################################################
.globl  ml_dsa_poly_ntt_inverse_avx2
.type   ml_dsa_poly_ntt_inverse_avx2,\@function,2
.align 32
ml_dsa_poly_ntt_inverse_avx2:
.cfi_startproc
...
___
    ...
    # offset arguments: coeff, l0, l1, l2,l3, l4
    &intt_levels0to4(0*4,    0*4,  128*4, 192*4, 224*4, 240*4);
    &intt_levels0to4(32*4,  16*4,  136*4, 196*4, 226*4, 241*4);
    &intt_levels0to4(64*4,  32*4,  144*4, 200*4, 228*4, 242*4);
    &intt_levels0to4(96*4,  48*4,  152*4, 204*4, 230*4, 243*4);
    &intt_levels0to4(128*4, 64*4,  160*4, 208*4, 232*4, 244*4);
    &intt_levels0to4(160*4, 80*4,  168*4, 212*4, 234*4, 245*4);
    &intt_levels0to4(192*4, 96*4,  176*4, 216*4, 236*4, 246*4);
    &intt_levels0to4(224*4, 112*4, 184*4, 220*4, 238*4, 247*4);
...
$code .= <<___;
.asciz  "ML-DSA NTT for x86_64, CRYPTOGAMS by <appro\@openssl.org>"
.align  64
___
